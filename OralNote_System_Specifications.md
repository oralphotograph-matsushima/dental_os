...
*   **詳細：** ログインやデータベース操作、カルテの閲覧機能を完全にシャットアウトし、利用者に「Windowsデスクトップ版（v1.3.0）インストーラー」のダウンロードを促します。
...
## 9. ビルド・デプロイ & クリーンアップ仕様 [v1.3.0 整備]
...
*   **構成内容:**
    ...
    3.  `scripts/`：Windows 用クリーンアップ本体である PowerShell スクリプト（`cleanup-oralnote.ps1`）を格納。
    4.  `OralNote_Clinic_Daily_Manual.html`：クリニックの日常運用マニュアル（HTML形式）。
...
### C. macOS上での Windows アプリケーションビルド
...
### D. 最新バージョン管理 & プロジェクトクリーンルール [v1.3.3 追加]

開発時の読み込み・ビルドパフォーマンスを最大化し、最新の成果物を明瞭に管理するため、以下のプロジェクト維持ルールを適用します。

1. **不要バージョンのクリーンアップルール**
   * 開発サーバーの起動時、あるいはVS Code等のエディタでのファイルシステム監視（Watch）において、`dist/` 配下にElectronビルド時の中間フォルダ（特に数万枚のDLLやバイナリが含まれる `dist/win-unpacked`）や過去の巨大な `.exe` / `.zip` インストーラーが残っていると、ファイルシステム監視の限界を超え、ページの読み込みやホットリロード、ビルド速度が劇的に低下（またはフリーズ）します。
   * そのため、**配布先に最新成果物を格納し終えた後は、プロジェクトルートの `dist/` 配下を完全にクリアするか、最新以外の古い世代（特に2世代以上前）のビルド成果物を完全に削除すること**を開発ルールとします。
   * `dist/` ディレクトリは `npm run build:win` でいつでも安全に全再生成されるため、容量削減と開発速度維持のため定期的に丸ごとクリーンアップ（`rm -rf dist/*` 等）を推奨します。

2. **最新バージョンの分かりやすさ維持**
   * 公式配布フォルダ（`Clinic_Distribution_Assets/2. OralNote/`）には、**原則として現在リリースされている最新バージョン（例: 1.3.4）および、万が一のための1つ前（1世代前、例: 1.3.3）の動作確認済みバージョンのみ**を保持します。
   * 2個前より古い過去のバージョンのインストーラー（例: 1.3.2 や 1.3.1 など）は、重複防止および管理ミス回避のため、速やかに公式配布フォルダおよびプロジェクトの `dist/` 配下から完全に削除します。これにより、現場のクリニック担当者がどのインストーラーを使うべきか迷わないようクリーンな状態を維持します。

3. **Gitへのインストーラー追跡防止ポリシー (Gitignore Rules)**
   * インストーラーやビルド成果物（`.exe`, `.zip`, `.dmg`, `.blockmap` 等）や一時的なリリースフォルダ（`*Release*/` 等）は容量が非常に大きく、Gitリポジトリの不要な肥大化やGitHub Desktop等のコミット選択時の手間を引き起こすため、リポジトリのルートにある `.gitignore` に登録して追跡対象外とします。
   * 過去にGitにコミットされて追跡対象になっていた古いインストーラー（例: `OralNote Setup 1.2.0.exe` 等）は、リポジトリのインデックスから完全に削除（`git rm`）してGitの管理対象から外し、GitHub Desktopに二度と変更候補として出現しないように徹底します。
