...
*   **詳細：** ログインやデータベース操作、カルテの閲覧機能を完全にシャットアウトし、利用者に「Windowsデスクトップ版（v1.1.1）インストーラー」のダウンロードを促します。
...
### A. 技工指示書メール送信機能 (Technician Order)
*   **概要：** 技工所を選択し、シェード写真や指示書写真を添付して直接メール送信。
*   **画像圧縮機能：** アップロード時のエラーやメールサーバー容量オーバーを防ぐため、送信前にフロントエンド（ブラウザ上）で画像を最大幅1600px、画質80%の**JPEGへ自動超高速圧縮**します。
